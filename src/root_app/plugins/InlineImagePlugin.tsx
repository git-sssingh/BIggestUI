...
interface IInsertInlineImageDialog {
  src: string;
  altText: string;
  showCaption: string;
  position: Position;
...
export function InsertInlineImageDialog({
  ...
  const [form] = Form.useForm<IInsertInlineImageDialog>();
  form.setFieldsValue({
    showCaption: "true",
    position: "left",
  });
  ...
  const handleOnClick = () => {
    ...
    const newPayload = {
      ...
      src: src?.url,
      showCaption:
        payload.showCaption && Boolean(payload.showCaption) ? true : false,
      height: "100%",
      width: "100%",
    };
    activeEditor.dispatchCommand(INSERT_INLINE_IMAGE_COMMAND, newPayload);
    onClose();
  };

  return (
    <Form form={form} autoComplete="off" onFinish={handleOnClick}>
      <DragAndDrop loadImage={loadImage} />
      <CTextField
        label="Alt Text"
        fieldProps={{
          textFieldProps: {
